...
class RunAPITest(JobServTest):
    ...
    def test_build_complete(self, requests_webhook, build_complete_email, storage):
        m = Mock()
        m.get_project_definition.return_value = json.dumps({
            'timeout': 5,
            'triggers': [
        ...
        requests_webhook.post.assert_called_once()
        call = requests_webhook.post.call_args_list[0]
